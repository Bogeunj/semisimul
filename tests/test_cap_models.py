...
def test_top_open_fraction_with_cap_hard_and_exp() -> None:
    ...
    np.testing.assert_allclose(
        hard, np.array([1.0, 1.0, 0.0], dtype=float), rtol=0.0, atol=1e-12
    )

    exp = top_open_fraction_with_cap(
        mask, tox, cap_eps_um=0.0, cap_model="exp", cap_len_um=0.01
    )
...
def test_exp_cap_reduces_robin_flux_as_tox_grows() -> None:
    ...
    open_low = top_open_fraction_with_cap(
        mask, np.zeros(grid.Nx), cap_eps_um=0.0, cap_model="exp", cap_len_um=0.01
    )
