...
def test_engine_matches_run_deck_data_for_same_mapping(tmp_path) -> None:
    ...
    state_direct = run_deck_data(
        deck, deck_path=deck_path, out_override=tmp_path / "direct"
    )
    result_engine = run_deck_mapping(
        deck, base_dir=deck_path.parent, out_override=tmp_path / "engine"
    )
...
def test_build_deck_from_ui_supports_arrhenius_schedule(tmp_path) -> None:
    ...
    state = run_deck_data(
        deck, deck_path=tmp_path / "ui.yaml", out_override=tmp_path / "run"
    )
    assert state.metrics is not None


@pytest.mark.unit
def test_build_deck_from_ui_rejects_invalid_schedule_text(tmp_path: Path) -> None:
    params = {
        "Lx_um": 1.0,
        "Ly_um": 0.3,
        "Nx": 81,
        ...
        "arrhenius_Ea_eV": 3.5,
        "arrhenius_T_C": 1000.0,
        "anneal_schedule_text": "not: [a valid list",
        "oxide_D_scale": 0.0,
        "cap_eps_um": 0.001,
        "cap_model": "hard",
        "cap_len_um": 0.01,
    ...
    with pytest.raises(ValueError, match=r"arrhenius\.schedule"):
        build_deck_from_ui(params, outdir=tmp_path / "ui")
