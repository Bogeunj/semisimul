...
def test_iso_area_cell_count_and_tri_linear_on_uniform_fields() -> None:
    ...
    assert (
        abs(
            iso_contour_area(high, grid, threshold_cm3=1.0, method="cell_count")
            - full_area_cell_count
        )
        < 1e-12
    )
    assert (
        abs(
            iso_contour_area(high, grid, threshold_cm3=1.0, method="tri_linear")
            - full_area_tri_linear
        )
        < 1e-12
    )
...
def test_iso_area_tri_linear_refines_with_grid() -> None:
    ...
    a_coarse = iso_contour_area(
        C_coarse, grid_coarse, threshold_cm3=0.37, method="tri_linear"
    )
    a_fine = iso_contour_area(
        C_fine, grid_fine, threshold_cm3=0.37, method="tri_linear"
    )
