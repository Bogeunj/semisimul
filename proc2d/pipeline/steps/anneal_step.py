...
def _anneal_diffusivity_segments(
    step: dict[str, Any], context: str
    ...
    if model != "arrhenius":
        raise DeckError(
            f"{context}: provide either D_cm2_s or diffusivity.model='arrhenius'."
        )

    D0 = to_float(
        required(diff_cfg, "D0_cm2_s", f"{context}.diffusivity"),
        "D0_cm2_s",
        f"{context}.diffusivity",
    )
    Ea = to_float(
        required(diff_cfg, "Ea_eV", f"{context}.diffusivity"),
        "Ea_eV",
        f"{context}.diffusivity",
    )
    ...
    if "schedule" in diff_cfg:
        ...
        for s_idx, seg in enumerate(schedule):
            if not isinstance(seg, dict):
                raise DeckError(
                    f"{context}.diffusivity.schedule[{s_idx}] must be a mapping."
                )
            seg_t = to_float(
    ...
    total_t = to_float(required(step, "total_t_s", context), "total_t_s", context)
    T_C = to_float(
        required(diff_cfg, "T_C", f"{context}.diffusivity"),
        "T_C",
        f"{context}.diffusivity",
    )
    ensure_positive(f"{context}.total_t_s", total_t, allow_zero=True)
...
def run_anneal_step(state: SimulationState, step: dict[str, Any], idx: int) -> None:
    ...
    cap_eps_um = to_float(
        step.get("cap_eps_um", 0.5 * state.grid.dy_um), "cap_eps_um", context
    )
    ...
    if cap_model == "exp":
        cap_len_um = to_float(step.get("cap_len_um", 0.01), "cap_len_um", context)
    ...
    try:
        ...
        raise DeckError(f"{context}: {exc}") from exc

    record_cfg = opt_mapping(step.get("record"), f"{context}.record")
    record_enable = bool(record_cfg.get("enable", False))
    record_every_s = to_float(
        record_cfg.get("every_s", dt_s), "every_s", f"{context}.record"
    )
    ...
    if record_enable:
        ...
        if bool(record_cfg.get("save_csv", True)):
            state.exports.append(
                save_history_csv(history_all, hist_outdir, filename="history.csv")
            )
        if bool(record_cfg.get("save_png", True)) and history_all:
            state.exports.append(
                save_history_png(history_all, hist_outdir, filename="history.png")
            )
